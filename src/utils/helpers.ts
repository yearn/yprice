...
export const toChecksumAddress = (address: string): string => {
  try {
    ...
    return '0x' + addr.split('').map(char => 
      char && parseInt(char, 16) >= 8 ? char.toUpperCase() : char
    ).join('');
  }
};
...
export const parseUnits = (value: string | number, decimals: number): bigint => {
  ...
  if (valueStr.match(/[eE]/)) {
    ...
    if (isNaN(num) || num < 1e-18) return BigInt(0);
    valueStr = num.toFixed(Math.max(decimals, 18));
  }
  
  const [intPart, decPart] = valueStr.split('.');
  if (!decPart) {
    try { return BigInt(intPart!) * factor; } catch { return BigInt(0); }
  }
  
  try {
    const decimalStr = decPart.padEnd(decimals, '0').slice(0, decimals);
    return BigInt(intPart || '0') * factor + 
...
export const formatUnits = (value: bigint, decimals: number): string => {
  ...
  return remainder === BigInt(0) ? integer.toString() : 
