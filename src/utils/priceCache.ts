...
import { filter, forEach, groupBy, mapValues, reduce, includes } from 'lodash';
...
export class PriceCache {
  ...
  getMany(chainId: number, addresses: string[]): Map<string, Price> {
    return reduce(addresses, (acc, address) => {
  ...
  cleanup(): void {
    const now = Date.now();
    const expired = filter(Array.from(this.cache.entries()), 
  ...
  getStats(): { total: number; chains: Map<number, number> } {
    ...
    const chains = new Map<number, number>(
      Object.entries(mapValues(grouped, arr => arr.length))
        .map(([chainId, count]) => [parseInt(chainId), count])
    );
  ...
  private getTokenType(symbol: string, address: string): TokenType {
    ...
    return {
      ...
      isLP: includes(lowerSymbol, 'lp') || 
            includes(lowerSymbol, '-') || 
            includes(lowerSymbol, 'uni-v'),
