...
export class VeloDiscovery {
  ...
  async discoverTokens(): Promise<TokenInfo[]> {
    ...
    try {
      ...
      logger.warn(
        `Velodrome discovery failed for chain ${this.chainId}: ${(errorMsg || 'Unknown error').substring(0, 100)}`,
      )
    }
  ...
  private async discoverFromContract(): Promise<TokenInfo[]> {
    ...
    try {
      ...
      const chainConfig = config[this.chainId as keyof typeof config] || {
        batchSize: 25,
        maxBatches: 30,
        timeout: 20000,
      }
      ...
      for (let i = 0; i < maxBatches; i++) {
        try {
          ...
          logger.debug(
            `Fetching batch ${i} (offset: ${offset}) from Sugar contract on chain ${this.chainId}`,
          )
      ...
      logger.debug(
        `Velodrome Sugar discovery completed: found ${uniqueTokens.size} unique tokens from ${tokens.length} total entries on chain ${this.chainId}`,
      )
    } catch (error: any) {
      logger.error(
        `Velodrome contract discovery failed for chain ${this.chainId}:`,
        error.message || error,
      )
    }
