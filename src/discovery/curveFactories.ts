  ...
  async discoverTokens(): Promise<TokenInfo[]> {
    ...
    for (const [factoryType, factoryAddress] of Object.entries(this.factories)) {
      try {
        ...
        logger.warn(
          `Chain ${this.chainId}: Failed to discover from Curve ${factoryType} factory:`,
          error.message,
        )
      }
    }
    ...
    try {
      ...
      try {
        ...
        if (error.message?.includes('returned no data') || error.message?.includes('reverted')) {
          logger.debug(
            `Chain ${this.chainId}: Curve ${factoryType} factory at ${factoryAddress} doesn't support pool_count, skipping`,
          )
          return tokens
        }
        throw error
      ...
      logger.debug(
        `Chain ${this.chainId}: Found ${poolAddresses.length} pools from Curve ${factoryType} factory`,
      )
